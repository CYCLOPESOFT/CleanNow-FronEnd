...
import { setAssistantSelected } from '../../Slices/authSlice';
      ...
      const handleAssistante = async () => {
        try {
          const response = await fetch(
            "https://localhost:7245/api/v1/Assistant",
            {
              method: "GET",
            }
          );
    ...
    return (
      <div className="bg-grayHome min-h-screen">
        ...
        <div className="mt-4 flex justify-center">
          {" "}
          {/* Ajuste de estilo para el filtro */}
          ...
          <main className="App-main container pt-5">
            ...
            {assistanst !== "" &&
              assistanst?.map((items, key) => (
                <Card className="assistant-card mb-8" id={items.id} key={items.id}>
                  <div className="flex flex-col-2 flex-row">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div>
                        <p className="text-lg">{items.age} años</p>
                        <p className="text-lg">{items.location}</p>
                      </div>
                      <p className="text-xl text-gray-500">
                        <span className="star-icon" style={{ color: "orange" }}>
                          ★
                        </span>
                        <span className="star-icon" style={{ color: "orange" }}>
                          ★
                      ...
                      <Link
                      onClick={()=>dispatch(setAssistantSelected(items.id))}
                        to={"/profileassistant"}
                        className="cursor-pointer text-purpleHome text-lg"
                      >
                        <span className="">
                          {items.experience} años de experiencia{" "}
                          <span style={{ color: "#6F00DE" }}>&gt;</span>
                        </span>
                      </Link>
                    </div>
                  </div>
        ...
        <footer className="h-28 sticky bottom-0 z-20 flex items-center justify-around bg-white py-8">
          ...
          <button href={"/ChatList"} className="flex items-center flex-col">
